...
#         snapshot = torch.load(snapshot_path, map_location=location)
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        # print(f"Параметры при подсчете loss: {text_tokens} - {text_tokens_lens}, {audio_features} - {audio_features_lens}, {params.train_stage}")

        # Ошибка в логе от 10.01 возникает в этой строке (CUDA out of memory)
...
def train_one_epoch(
    ...
    Args:
      ...
      rank:
        ...
        be set to 0.
    """
    print(f"Ресурсы GPU compute_loss_start: {torch.cuda.mem_get_info()}")
    clear_gpu_memory()
    print(f"Ресурсы GPU compute_loss_end: {torch.cuda.mem_get_info()}")
    ...
    while True:
        try:
            ...
            print("Reaches end of dataloader.")
            break
        ...
        if params.batch_idx_train % params.valid_interval == 0:
            ...
            print(
                f"Epoch {params.cur_epoch}, validation: {valid_info}, global_rank: {global_rank}"
            )
            print(
...
def run(args):
    ...
    print(f"Ресурсы GPU: {torch.cuda.mem_get_info()}")

    params = get_params()
    print(f"Параметры запуска обучения {params}")
    ...
    model, codec, vocos = get_model(local_rank)
    ...
    model_avg: Optional[nn.Module] = None
    if local_rank == 0 and params.average_period > 0:
    ...
    if checkpoints and "grad_scaler" in checkpoints:
        print("Loading grad scaler state dict")
    ...
    for epoch in range(params.start_epoch, params.num_epochs + 1):
        ...
        print(f"Ресурсы GPU после обучения эпохи: {torch.cuda.mem_get_info()}")
...
def display_and_save_batch(
    ...
    print(f"Saving batch to {filename}")
